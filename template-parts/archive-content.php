<article class="post block block--inset block--list<?php if(is_sticky()) echo ' sticky';?>">
    ...
    <h2 class="post-title block-title post-featured" itemprop="headline">
        <?php if( is_sticky()) : ?>
            <img src="<?php echo(get_bloginfo('template_directory').'/source/img/sticky-top.svg') ?>" class="sticky-icon" title="A sticky post"/><a href="<?php the_permalink();?>"><?php the_title();?></a>
        <?php else : ?>
